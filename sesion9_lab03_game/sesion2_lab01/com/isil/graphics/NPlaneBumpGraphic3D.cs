...
namespace Sesion2_Lab01.com.isil.graphics {
    public class NPlaneBumpGraphic3D
    {
        ...
        private Shader3DProgramBump mShader;
        private NTexture2D mTexture2D;
        private NTexture2D mTexture2D_Bump;
        ...
        public NPlaneBumpGraphic3D(string path, string bump_path, float x, float y, float z, int numTiles)
        {
            this.X = x;
            this.Y = y;
            this.Z = z;
            ...
            if (numTiles > 0)
            {
                ...
                mTexture2D_Bump = new NTexture2D(NativeApplication.instance.Device,
                    SharpDX.Direct3D11.TextureAddressMode.Mirror);
                mTexture2D_Bump.Load(bump_path);
            }
            else {
                mTexture2D = new NTexture2D(NativeApplication.instance.Device);
                mTexture2D.Load(path);
            ...
            mShader = new Shader3DProgramBump(NativeApplication.instance.Device);
            mShader.Load("Content/Fx_PrimitiveTexture3DBump.fx");
        ...
        public void Draw(RenderCamera camera, int dt)
        {
            ...
            mShader.Draw(camera.transformed, mTexture2D, mTexture2D_Bump);
